...
Push-Location $frontendDir
try {
    $frontendProcess = Start-Process -FilePath "npm.cmd" -ArgumentList "run dev" -RedirectStandardOutput (Join-Path $PWD.Path "logs\frontend.log") -RedirectStandardError (Join-Path $PWD.Path "logs\frontend_error.log") -PassThru
